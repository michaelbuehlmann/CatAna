...
//                std::cout << "\t " << lower_z << ",\t" << upper_z << ": ";
            exit_code = gsl_integration_qag(F, lower_z, upper_z, 0, 100*double_t_lim::epsilon(), 1000, 2, workspace,
                    &temp,
                    &error);
...
SBesselTransformedFunction::SBesselTransformedFunction(std::function<double_t(double_t)> f, unsigned int l, double_t Rmax)
