...
#pragma omp parallel for if(parallel) schedule(dynamic, 5)
        for(int i=0; i<nrings; ++i){
            for(unsigned short m=0; m<=l; ++m){
                ylm_im(i,m) = std::conj(boost::math::spherical_harmonic(l, m, ring_info_container[i].theta, ring_info_container[i].phase));
            }
        }
...
#pragma omp for schedule(dynamic, 5)
                for (int i = 0; i<nrings; ++i) {
                    sfb_ringdecomp_kernel(l, k_n, pix_obj_cont, sph_bessel_l, f_lmn_m_private, &ylm_im(i,0), ring_info_container[i]);
